...
import { ConversationTypes } from "@/contexts/AppStateContext";
...
export function ClientPage() {
  ...
  useEffect(() => {
    ...
    const isVoiceMode = conversationType === ConversationTypes.VOICE_TO_VOICE;
    const isTeacherMode = conversationType === ConversationTypes.TEACHER;

    const newClient = new RTVIClient({
      enableCam: false,
      // Enable mic for both voice-to-voice and teacher mode
      enableMic: isVoiceMode || isTeacherMode,
      // Use WebSocket transport for voice-to-voice, Daily for others
      transport: isVoiceMode
        ? new GeminiLiveWebsocketTransport({ api_key: geminiApiKey })
        : new DailyTransport(),
      params: {
        baseUrl: import.meta.env.VITE_SERVER_URL,
        endpoints: {
          // Use appropriate endpoint based on mode
          connect: isTeacherMode ? "/bot/teacher-connect" : "/bot/connect",
          action: "/bot/action",
        },
        requestData: {
          bot_profile: isVoiceMode ? "voice-to-voice" : "text-voice",
          conversation_id: "",
        },
      },
    });
  ...
  return (
    <RTVIClientProvider client={client!}>
      {!conversationId && !conversationType && (
        <div className="flex flex-col gap-6 p-4">
          ...
          <div className="flex flex-col gap-4">
            {/* Add Teacher Mode Button */}
            <Button
              className="relative h-full flex flex-col border border-transparent bg-origin-border borderClip bg-cardBorder justify-between gap-2 max-w-72 lg:max-w-80 text-wrap rounded-3xl p-4 lg:p-6 shadow-mid hover:shadow-long hover:bg-cardBorderHover transition-all text-base"
              disabled={!webrtcEnabled}
              onClick={() => setConversationType(ConversationTypes.TEACHER)}
            ...
            >
              ...
              <div className="flex items-center justify-center bg-violet-100 text-violet-400 rounded-full">
                <GraduationCapIcon className="h-20 w-20 p-4" />
              </div>
              <div className="flex flex-col gap-2">
                ...
                <span className="font-light text-neutral-500">
                  Interactive teaching sessions with structured flows and voice interaction
                </span>
              </div>
      ...
      {/* Show TeacherMode component when in teacher mode */}
      {conversationType === ConversationTypes.TEACHER && !conversationId && <TeacherMode />}

      {/* Show chat interface only after conversation is created */}
      {conversationId && (
